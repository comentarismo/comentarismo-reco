    ...
    it 'recommendations_per_neighbour should stop one persons recommendations eliminating the other recommendations', ->
      ...
      .then (ger) ->
        bb.all([
          ...
          ger.event(ns, 'p2','buy','x', created_at: moment().subtract(2, 'days').utc(), expires_at: tomorrow),

          ger.event(ns, 'p3','view','a'),
          ger.event(ns, 'p3','buy','l', created_at: moment().subtract(3, 'hours').utc(), expires_at: tomorrow),
          ger.event(ns, 'p3','buy','m', created_at: moment().subtract(2, 'hours').utc(), expires_at: tomorrow),
          ger.event(ns, 'p3','buy','n', created_at: moment().subtract(1, 'hours').utc(), expires_at: tomorrow)
        ])
